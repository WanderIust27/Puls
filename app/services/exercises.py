...
SEED_EXERCISES: list[dict[str, Any]] = [
    ...
    # Der Push/Pull-Split hat eine Luecke sichtbar gemacht: Auf der Zugseite
    # standen neun Uebungen, auf der Druckseite fuenf. Bei Ganzkoerper faellt
    # das nicht auf, weil jede Einheit ohnehin reihum geht — an einem reinen
    # Push-Tag ist die Einheit dann eine halbe Stunde zu kurz. Die vier hier
    # stehen so in 02_Training_Gym.md, Template A und B.
    dict(name="Brustpresse (Maschine)", muscle_group="chest", equipment="machine",
         weight_kg=40, weight_increment=5, target_reps=12, rep_min=8, rep_max=12,
         sets=3, rest_s=90, slot="main", priority=2,
         garmin_category="BENCH_PRESS", garmin_exercise="MACHINE_BENCH_PRESS",
         sort_order=26, notes="Griff auf Brusthöhe, Schulterblätter hinten",
         aliases=["brustpresse", "chest press", "maschinen bankdruecken",
                  "brustdruecken maschine"]),
    dict(name="Schrägbankdrücken", muscle_group="chest", equipment="dumbbell",
         weight_kg=15, weight_increment=5, target_reps=10, rep_min=8, rep_max=12,
         sets=3, rest_s=90, slot="main",
         garmin_category="BENCH_PRESS", garmin_exercise="INCLINE_DUMBBELL_BENCH_PRESS",
         sort_order=27, notes="Bank auf 30 Grad — flacher trifft die Brust besser",
         aliases=["schraegbank", "schraegbankdruecken", "incline press",
                  "incline bench", "schrägbank kurzhantel"]),
    dict(name="Schulterdrücken (Maschine)", muscle_group="shoulders",
         equipment="machine",
         weight_kg=25, weight_increment=5, target_reps=10, rep_min=8, rep_max=12,
         sets=3, rest_s=90, slot="main", priority=2,
         garmin_category="SHOULDER_PRESS", garmin_exercise="MACHINE_SHOULDER_PRESS",
         sort_order=41, notes="Nicht ins Hohlkreuz ausweichen",
         aliases=["schulterdruecken", "shoulder press", "overhead press",
                  "schulterpresse", "military press"]),
    dict(name="Trizepsdrücken über Kopf am Kabel", muscle_group="arms",
         equipment="cable",
         weight_kg=15, weight_increment=5, target_reps=12, rep_min=10, rep_max=15,
         sets=3, rest_s=60, slot="main",
         garmin_category="TRICEPS_EXTENSION",
         garmin_exercise="CABLE_OVERHEAD_TRICEPS_EXTENSION",
         sort_order=91, notes="Ellbogen bleiben eng, nur der Unterarm bewegt sich",
         aliases=["ueberkopf trizeps", "overhead trizeps", "trizeps ueber kopf",
